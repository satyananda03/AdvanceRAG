...
# WebUI-recognized status value.  RAGAnything's own ``ready`` / ``handling``
# states fall through the dashboard's ``documentManager.status`` switch to
# the ``default`` branch and render in red as "failed".  Using ``parsing``
# (a native LightRAG DocStatus value) means the scaffold record shown while
# RAGAnything is extracting content matches the label + color LightRAG's
# own pipeline uses during its parse stage.
...
class ProcessorMixin:
    ...
    async def _upsert_doc_status(
        ...
        **updates,
        ...
        # Always persist the caller-supplied tag, even on records created by
        # a different code path (e.g. ``lightrag.ainsert``), so scheme_name
        # remains authoritative for whoever passed it in.
        if scheme_name is not None:
            updated_doc_status["scheme_name"] = scheme_name
    ...
    async def _process_multimodal_content_individual(
        ...
        if all_chunk_results:
            ...
            await merge_nodes_and_edges(
                ...
                global_config=self.lightrag._build_global_config(),
    ...
    async def process_document_complete_lightrag_api(
        ...
        **kwargs,
        ...
        # Use full path or basename based on config
        file_name = self._get_file_reference(file_path)
        doc_pre_id = f"parsing-{file_name}"
        pipeline_status = None
        pipeline_status_lock = None
        current_doc_status = {}  # initialised here so the except block can always unpack it
        # ``handed_off`` flips to True once the pre-doc tracker is released and
        # the real ``doc_id`` (created by ``lightrag.ainsert``) owns doc_status.
        # This routes failure updates to the correct record instead of
        # resurrecting the deleted pre-doc entry.
        handed_off = False
        ...
        try:
            ...
            # Pre-doc tracker: created once with valid timestamps so PG-backed
            # doc_status doesn't emit ``Unable to parse doc status datetime``
            # warnings.  It is released (deleted) once parsing succeeds, so
            # ``lightrag.ainsert`` can create the real ``doc_id`` record
            # without hitting its own filename dedup.
            #
            # Uses ``_PARSING_STATUS`` (``"parsing"``) instead of
            # ``DocStatus.READY``/``HANDLING`` because the dashboard WebUI's
            # status switch has no case for the RAGAnything-specific values
            # and would render them as "failed" (default branch).
            timestamp = self._current_doc_status_timestamp()
            ...
            if not current_doc_status:
                ...
                await self.lightrag.doc_status.upsert(
                    {doc_pre_id: current_doc_status}
                )
                # Re-read to pick up any storage-layer defaults (e.g. chunks_list).
                current_doc_status = (
                    await self.lightrag.doc_status.get_by_id(doc_pre_id)
                    or current_doc_status
                )
            ...
            # Handoff: release the pre-doc tracker before invoking ainsert.
            # LightRAG's ingest pipeline flags any pre-existing doc_status
            # record that shares the file basename (or the target ``doc_id``)
            # as a duplicate -- which is exactly what this scaffold record
            # would look like.  Removing it here lets ainsert create and own
            # the authoritative ``doc_id`` entry.
            await self.lightrag.doc_status.delete([doc_pre_id])
            await self.lightrag.doc_status.index_done_callback()
            handed_off = True
            ...
            # ainsert does not persist ``scheme_name`` -- reapply here so the
            # caller-provided tag survives on the doc_status entry it created.
            if scheme_name is not None:
                await self._upsert_doc_status(
            ...
            # Route the failure to whichever record currently owns the doc:
            # the pre-doc tracker before handoff, the real ``doc_id`` after.
            try:
                if handed_off and doc_id is not None:
                    await self._upsert_doc_status(
                        doc_id,
                        file_name,
                        status=DocStatus.FAILED,
                ...
                self.logger.error(
                    f"Failed to persist failure doc_status for {file_path}: "
                    f"{status_error}"
                )
