...
class BaseModalProcessor:
    ...
    def __init__(
        ...
        # Build the global config the same way LightRAG does internally
        # (_build_global_config) so it carries the runtime-injected
        # `role_llm_funcs` / `llm_cache_identities` maps. A plain asdict(lightrag)
        # omits those (they live in the private _role_llm_states map, not in
        # dataclass fields), which makes downstream extract_entities /
        # merge_nodes_and_edges raise KeyError: 'role_llm_funcs'.
        build_global_config = getattr(lightrag, "_build_global_config", None)
        if callable(build_global_config):
            print("CALLABLE build global config")
            self.global_config = build_global_config()
            # asdict() inside _build_global_config converts the EmbeddingFunc
            # object into a plain dict; restore the real callable so it matches
            # LightRAG's own post-build fix-up.
            self.global_config["embedding_func"] = self.embedding_func
        else:
            print("OLDER LIGHTRAG")
            # Fallback for older LightRAG builds without _build_global_config.
...
class ImageModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            section_path = content_data.get("_section_path", "")
            caption_str = ", ".join(str(c) for c in captions) if captions else ""
            footnote_str = ", ".join(str(f) for f in footnotes) if footnotes else ""
            placeholder_parts = ["[Image analysis unavailable]"]
            if section_path:
                placeholder_parts.append(f"Section: {section_path}")
            if caption_str:
                placeholder_parts.append(f"Captions: {caption_str}")
            if footnote_str:
                placeholder_parts.append(f"Footnotes: {footnote_str}")
            placeholder_caption = " | ".join(placeholder_parts)
            summary_source = caption_str or section_path or "Image analysis unavailable"
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
            ...
            return placeholder_caption, fallback_entity
...
class TableModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            # Fallback: emit clean placeholder text (not the raw stringified
            # modal_content dict) so the chunk and entity summary stay clean
            # and extract_entities on the chunk does not pull noise entities.
            content_data = modal_content if isinstance(modal_content, dict) else {}
            table_caption = normalize_caption_list(
                content_data.get("table_caption")
            )
            table_footnote = normalize_caption_list(
                content_data.get("table_footnote")
            )
            caption_str = ", ".join(str(c) for c in table_caption) if table_caption else ""
            footnote_str = ", ".join(str(f) for f in table_footnote) if table_footnote else ""
            placeholder_parts = ["[Table analysis unavailable]"]
            if caption_str:
                placeholder_parts.append(f"Caption: {caption_str}")
            if footnote_str:
                placeholder_parts.append(f"Footnotes: {footnote_str}")
            placeholder_caption = " | ".join(placeholder_parts)
            summary_source = caption_str or "Table analysis unavailable"
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
...
class EquationModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            content_data = modal_content if isinstance(modal_content, dict) else {}
            equation_text, equation_format = get_equation_text_and_format(content_data)
            eq_text_str = str(equation_text) if equation_text else ""
            placeholder_parts = ["[Equation analysis unavailable]"]
            if equation_format:
                placeholder_parts.append(f"Format: {equation_format}")
            if eq_text_str:
                placeholder_parts.append(f"Equation: {eq_text_str[:200]}")
            placeholder_caption = " | ".join(placeholder_parts)
            summary_source = eq_text_str or "Equation analysis unavailable"
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
...
class GenericModalProcessor(BaseModalProcessor):
    ...
    async def generate_description_only(
        ...
        try:
            ...
            # modal_content dict) so the chunk and entity summary stay clean
            # and extract_entities on the chunk does not pull noise entities.
            placeholder_caption = f"[{content_type} analysis unavailable]"
            fallback_entity = {
                "entity_name": entity_name
                if entity_name
                else f"{content_type}_{compute_mdhash_id(str(modal_content))}",
                "entity_type": content_type,
                "summary": placeholder_caption,
            }
            return placeholder_caption, fallback_entity
